...
class Tickets(object):
    ...
    def printtable(self,rows):
        ...
        info = "车次 出发站 目的地 终点站 出发时间 到达时间 历时 商务座 特等座 一等座 二等座 高级 软卧 动卧 硬卧 软座 硬座 无座"
        info = info.split(" ")
        x = PrettyTable(info)
        for row in rows:
            x.add_row(row)
